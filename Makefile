...
CONFORMANCE_PROTO_REF := 7d7bb3c6e58532d681180bd7df26a2f8ecbb016d
CONFORMANCE_RUNNER_TAG := v1.0.0-rc4
...
testconformance: ## Run all conformance tests
	...
	PATH="$(abspath $(BIN)):$(PATH)" connectconformance -vv --conf ./Tests/ConformanceClient/InvocationConfigs/urlsession.yaml --mode client $(BIN)/ConnectConformanceClient httpclient=urlsession
	PATH="$(abspath $(BIN)):$(PATH)" connectconformance -vv --conf ./Tests/ConformanceClient/InvocationConfigs/nio.yaml --mode client $(BIN)/ConnectConformanceClient httpclient=nio
