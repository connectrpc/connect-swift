...
/// Interceptor that can be used to retry stream connections automatically.
public final class StreamRetryInterceptor: @unchecked Sendable {
    ...
    private var initiateStream: (() -> Void)!
...
extension StreamRetryInterceptor: StreamInterceptor {
    ...
    public func handleStreamResult<Message>(
        _ result: StreamResult<Message>,
        proceed: @escaping (StreamResult<Message>) -> Void
    ) {
        guard case .complete(let code, _, _) = result,
              code != .ok,
              self.codesToRetry.contains(code),
